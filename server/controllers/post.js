...
const createPosts = asyncHandler(async (req, res) => {
  ...
  return res.json({
    ...
    mes: response[1]
      ...
      : 'Tựa đề bài đăng không được trùng nhau',
  });
});
...
const deletedPost = asyncHandler(async (req, res) => {
  ...
  return res.json({
    ...
    createdPost: response > 0 ? 'Deleted' : 'Không tìm thấy bài viết',
  });
});
  ...
  if (q) {
    query = {
      [Op.or]: [
        ...
        { address: { [Op.substring]: q.trim() } },
        { ['$user.name$']: { [Op.substring]: q.trim() } },
      ],
    };
  }
  query.postedBy = req.user.uid;
...
const getPostsByAdmin = asyncHandler(async (req, res) => {
  ...
  if (q && typeof q === 'object') {
    query = {
      [Op.or]: [
        { title: { [Op.substring]: q[0].trim() } },
        { address: { [Op.substring]: q[0].trim() } },
        { ['$user.name$']: { [Op.substring]: q[0].trim() } },
      ],
    };
  }
  ...
  if (price)
    query.price = { [Op.between]: price[0].split(',')?.map((el) => +el) };
  if (distance)
    query.distance = { [Op.between]: distance[0].split(',')?.map((el) => +el) };
...
const ratings = asyncHandler(async (req, res) => {
  ...
  if (alreadyVote) {
    ...
    return res.json({
      success: response ? true : false,
      data: response ? response : 'Không tìm thấy bài viết.',
    });
  } else {
...
const getPostById = asyncHandler(async (req, res) => {
  ...
  return res.json({
    success: response ? true : false,
    post: response ? response : 'Không tìm thấy bài viết',
  });
});
...
const getDashboard = asyncHandler(async (req, res) => {
  ...
  return res.json({
    success: [rent, eatery, other, user, views, postCount, userCount]
      ? true
      : false,
